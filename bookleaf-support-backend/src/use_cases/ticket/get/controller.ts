...
import { validateGetTicket } from "./validator";
import { GetTicketDtoConverter } from "./dto";
import { GetTicketUseCase } from "./usecase";

export class GetTicketController {
  constructor(private readonly useCase: GetTicketUseCase) {}
  ...
  private handle = async (
    ...
    const { data, errors } = validateGetTicket(req.params);
    if (!data) {
      ResponseUtil.error(res, "Validation failed", 400, errors);
      return;
    }

    const dto = new GetTicketDtoConverter(req.user, data).getDtoObject();
