...
export class UpdateCategoryUseCase {
  async execute(
  ...
  ): Promise<UseCaseResult<UpdateCategoryResult>> {
    ...
    const aiOverridden =
      existing.aiMetadata?.source === "AI" && before !== dto.category;

    const updated = await ticketQueries.updateCategory(
      dto.ticketId,
      dto.category,
    );
    ...
    try {
      await activityQueries.log({
        ...
        after: { category: dto.category, aiOverridden },
      });
    } catch (err) {
      logger.warn(
        { err, ticketId: dto.ticketId },
        "activity log failed: CATEGORY_CHANGED",
      );
    }
